...
export default function loadCOrdersTable() {
    return new ETLTask()
    .fromSQLDatabase(connections.mssql, `SELECT p.ProductID, e.EmployeeID, s.SupplierID, c.CustomerID, CONVERT(VARCHAR(10), o.OrderDate, 111) AS OrderDate, SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) AS TotalPrice
